...
type SeedItem = {
  title: string
  status?: Status
  tags?: string[]   // [priority, area] e.g., ['P0 — Pre-launch', 'Indexation']
  notes: string     // Why / How / Done / Owner
}
...
// ---------- Your list as cards with rich notes ----------
export const tasks: SeedItem[] = [
  // P0 — Pre-launch MUST-FIX (today)
  {
    title: 'Block staging from indexation',
    tags: [P0, 'Indexation'],
    notes: `Owner: Dev FE / Ops
...
Done: GSC can fetch production sitemaps; no staging URLs present.`
  ...
  // P1 — Launch window (launch day → +72h)
  {
...
Done: Direct hits to staging require auth or 301 to prod; no canonical pointing to staging.`
  },
  {
    ...
    tags: [P1, 'Content'],
    notes: `Owner: Content / SEO
Why: Marketplace-style content confuses Google's understanding; you are a manufacturer.
How:
- 410 the legacy blog about buying/selling personalised plates

mkdir -p src/features/seo
cat > src/features/seo/seedFromList.ts <<'EOF'
import { supabase } from '@/lib/supabase'
...
- 410 the legacy personalised plates blog (or keep with noindex + contextual rewrite if truly needed)
...
Done: 404/410 return correct codes; recovery links function.`
  ...
  // QA checklist (single card that you can tick with subtasks in notes)
  {
    title: 'QA — template pass/fail checks',
    tags: [P1, 'QA'],
    notes: `Owner: SEO / Dev FE
...
// Ensure project exists (scoped by current user via RLS)
...
// Seed from the list: insert if missing by title; update notes if empty
